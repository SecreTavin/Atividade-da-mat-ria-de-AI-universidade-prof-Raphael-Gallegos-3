...
def mapa_percorrido():
    ...
    grid = [list(linha) for linha in mapa_matriz]

    inicio, objetivo = (0,0), (14,14)

#BFS
    caminho_bfs, explorados_bfs = bfs(grid, inicio, objetivo)
    print('=== BFS ===')
    if caminho_bfs:
        print(f'caminho encontrado! passos: {len(caminho_bfs)-1}')
        print(f'Nós explorados: {explorados_bfs}')
        print_mapa(grid, caminho_bfs)
        salvar_mapa_txt(grid, caminho_bfs, 'mapa_BFS.txt')
        print("Mapa BFS salvo em 'mapa_BFS.txt'")
    else:
        print('Nenhum caminho encontrado!')

#A*
    caminho_a, explorados_a = a_man(grid, inicio, objetivo)
    print('=== A* ===')
    if caminho_a:
        print(f'caminho encontrado! passos: {len(caminho_a)-1}')
        print(f'Nós explorados: {explorados_a}')
        print_mapa(grid, caminho_a)
        salvar_mapa_txt(grid, caminho_a, 'mapa_Astar.txt')
